...
![2D 星座早見から 3D プラネタリウムへ切り替え、ドラッグで見回す様子](docs/preview.gif)
...
| `city` | 観測地 (都市プリセット名) | `tokyo`、`osaka`、`sapporo`、`nagoya`、`fukuoka`、`naha`、`london`、`newyork`、`sydney`、`honolulu` |
| `view` | 初期ビュー | `2d` / `3d` |

いずれも省略でき、省略時と不正値のときは既定の挙動 (現在日時または当夜 20:00、東京、2D)
になります。`lat` / `lon` と `city` を両方指定した場合は `lat` / `lon` を使います。
`t` は **UTC オフセットを付ける形式を推奨** します。省略するとブラウザの
タイムゾーンで解釈されるため、実行環境によって指す時刻が変わります。
URL に生の `+` を書くとクエリ文字列の復号で空白になりますが、
末尾のオフセットに限り補正して解釈します (`%2B` と書けば確実です)。

表示中の日時とビューは `history.replaceState` で URL にも反映されるため、
アドレスバーをそのままコピーして共有できます。設定パネルの「この空の URL を作る」からも
同じ URL を取得できます。

**観測地の扱い**: **緯度経度はアドレスバーには出しません。**
出してしまうと、注意書きと同意のある共有ダイアログを通らずに、アドレスバーを
コピーするだけで座標が渡ってしまうためです。観測地が都市プリセットのときだけ、
座標ではなく名前で `city=tokyo` のように反映します
(URL パラメータで座標を渡して開いた場合も、観測地には反映しますがアドレスバーには残しません)。

緯度経度が入った URL は、共有ダイアログで明示的に選んだときだけ生成します。
都市プリセット以外の観測地 (「現在地を取得」と手入力) では注意書きを表示し、
既定では含めません。含めない場合、共有先では既定の観測地 (東京) で表示されます。
